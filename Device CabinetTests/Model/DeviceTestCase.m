...
- (void)testInitWithJson {
    ...
    NSDictionary *json = @{@"device_name" : storedDevice.deviceName, @"device_id" : storedDevice.deviceUdId, @"category" : storedDevice.type, @"system_version" : storedDevice.systemVersion, @"is_booked" : storedDevice.isBookedByPerson ? @"YES" : @"NO", @"device_type" : storedDevice.deviceModel, @"image_url" : @"", @"is_booked" : @"No", @"person_id" : @""};
    ...
    XCTAssertEqualObjects(device.deviceModel, storedDevice.deviceModel, @"deviceType from device initialized with json should be same like storedDevice");
...
- (void)testToDictionary {
    ...
    XCTAssertEqualObjects(device.deviceModel, [json valueForKey:@"device_type"], @"deviceType should be same in json");
